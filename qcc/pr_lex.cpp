...
def_t* def_for_type[8] = {&def_void,     &def_string, &def_float,
                          &def_vector,   &def_entity, &def_field,
...
void PR_LexGrab(void) {
    ...
    else if (!strcmp(pr_token, "cd") || !strcmp(pr_token, "origin") ||
             !strcmp(pr_token, "base") || !strcmp(pr_token, "flags") ||
...
type_t* PR_ParseType(void) {
    ...
    if (PR_Check(".")) {
        ...
        new_type.type = ev_field;
        new_type.aux_type = PR_ParseType();
        return PR_FindType(&new_type);
    }
    ...
    // function type
    memset(&new_type, 0, sizeof(new_type));
    new_type.type = ev_function;
    new_type.aux_type = type; // return type
    new_type.num_parms = 0;
    if (!PR_Check(")")) {
        if (PR_Check("..."))
            new_type.num_parms = -1; // variable args
        else
            do {
                type = PR_ParseType();
                name = PR_ParseName();
                strcpy(pr_parm_names[new_type.num_parms], name);
                new_type.parm_types[new_type.num_parms] = type;
                new_type.num_parms++;
            } while (PR_Check(","));
