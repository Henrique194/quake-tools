...
int main(int argc, char** argv) {
    ...
    if (CheckParm("-validate")) {
        FILE* original = fopen("./progs.dat", "rb");
        FILE* modified = fopen("../progs.dat", "rb");
        const int length = filelength(original);
        const int modified_length = filelength(modified);
        if (length != modified_length) {
            printf("Different progs!\n");
            printf(
                "Original prog has size %d while modified prog has size %d!\n",
                length, modified_length);
            fclose(original);
            fclose(modified);
            return 0;
        }
        char* original_buf = (char*) malloc(length);
        char* modified_buf = (char*) malloc(length);
        SafeRead(original, original_buf, length);
        SafeRead(modified, modified_buf, length);
        for (int i = 0; i < length; i++) {
            if (original_buf[i] != modified_buf[i]) {
                printf("Different progs!\n");
                printf("Different byte at %d!\n", i);
                return 0;
            }
        }
        fclose(original);
